...
class Solution {
    public List<List<Integer>> combinationSum2(int[] candidates, int target) {
        ...
        TargetSum2(0, candidates, new ArrayList<>(), res, target); // data structure initiall empty
        return res;
    }
    ...
    public static void TargetSum2(int idx, int[] nums, List<Integer> curr, List<List<Integer>> res, int target) {
        ...
        for (int i = idx; i < nums.length; i++) { // i to n combination

            // Skip duplicates
            if (  (i == idx || nums[i] != nums[i - 1]) && nums[i] <= target) { // pehla idx, uniq comb, less then t
